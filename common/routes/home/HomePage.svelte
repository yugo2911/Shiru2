<script context='module'>
  ...
  export async function loadSectionData(index) {
    ...
    resolvedCatalog.set(items.map(item => ({
      ...item.data,
      progress: item.data.mediaListEntry?.progress || 0
    })))
  }
...
<svelte:head>
  <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;900&family=JetBrains+Mono:wght@800&display=swap" rel="stylesheet">
</svelte:head>
...
<div class="home-theater">
  ...
  <main class="content-gate">
    {#if selectedAnime}
        ...
        <div class="meta-block" in:fade={{ duration: 300 }}>
          ...
          <div class="stat-grid">
            ...
            <div class="stat">
              ...
              <span class="value">{selectedAnime.progress}<small>/{selectedAnime.episodes || '?'}</small></span>
            </div>
          </div>
      ...
      {/key}
    {:else}
      <div class="loading-wrap" in:fade>
...
<style>
  ...
  /* Faster, hardware-accelerated background */
  .theater-bg { 
    position: absolute; 
    inset: 0; 
  ...
  .vignette { 
    ...
    background: linear-gradient(to top, #050505 15%, transparent 100%), 
                linear-gradient(to right, #050505 10%, transparent 60%); 
    z-index: 0; 
  }
  ...
  .scroll-wrapper { display: flex; gap: 1rem; overflow-x: auto; scrollbar-width: none; -ms-overflow-style: none; }
