...
{#if matches('Disable Miniplayer', 'miniplayer', 'Player Settings')}
...
{/if}
{#if !$playPage}
{#if matches('Auto-Hide Miniplayer', 'miniplayer shelve', 'Player Settings')}
...
{#if matches('Default Subtitle Font', 'font to use when the current loaded video', 'Subtitle Settings')}
...
{/if}
{#if matches('Missing Subtitle Fonts', "finds and loads fonts that are missing", 'Subtitle Settings')}
...
{/if}
{/if}
{#if matches('Fast Subtitle Rendering', 'Disables blur when rendering subtitles', 'Subtitle Settings')}
...
{#if matches('Jimaku API', 'Jimaku.cc', 'Subtitle Integrations')}
...
{/if}
{#if matches('Subtitle Render Resolution', 'render subtitles at', 'Subtitle Integrations')}
...
{#if matches('Preferred Subtitle Language', 'subtitle language to automatically select', 'Language Settings')}
...
{#if matches('Autoplay Next Episode', 'Automatically starts playing next episode', 'Playback Settings')}
...
{/if}
{#if matches('Pause On Lost Focus', 'Pauses/Resumes video playback when tabbing', 'Playback Settings')}
...
{/if}
{#if matches('Auto-Complete Episodes', 'Automatically marks episodes as complete', 'Playback Settings')}
...
{#if matches('Auto-Complete Threshold', 'percentage of an episode that must be watched', 'Playback Settings')}
...
{/if}
{/if}
{#if matches('Deband Video', 'Reduces banding on dark and compressed videos', 'Playback Settings')}
...
{/if}
{#if matches('Seek Duration', 'skip forward or backward when using the seek buttons', 'Playback Settings')}
...
{/if}
{#if matches('Chapter Source', 'chapter source to use during video playback', 'Playback Settings')}
...
{/if}
{#if matches('Auto-Skip Intro', 'automatically skip intro and outro', 'Playback Settings')}
...
{#if matches('External Player', 'external video player to play video', 'External Player Settings')}
...
{#if matches('External Video Player', 'Executable for an external video player', 'External Player Settings')}
