...
{/if}
{#if matches('DNS Over HTTPS', 'DNS Over HTTPS', 'DNS')}
...
{/if}
{#if matches('DNS Over HTTPS URL', 'URL to use for querying DNS', 'DNS')}
...
{/if}
{#if matches('Download Location', 'folder used to store torrents', 'Client')}
...
{/if}
{#if matches('Persist Files', 'Keeps torrents files instead of deleting them', 'Client')}
...
{/if}
{#if matches('Streamed Download', 'single file that is currently being watched', 'Client')}
...
{/if}
{#if matches('Transfer Speed Limit', 'Download/Upload speed limit for torrents', 'Client')}
...
{/if}
{#if matches('Max Number of Connections', 'peers per torrent', 'Client')}
...
{/if}
{#if matches('Seeding Limit', 'torrents that can be seeded at the same time', 'Client')}
...
{/if}
{#if matches('Torrent Port', 'Port used for Torrent connections', 'Client')}
...
{/if}
{#if matches('DHT Port', 'Port used for DHT connections', 'Client')}
...
{/if}
{#if matches('Disable DHT', 'Distributed Hash Tables', 'Client')}
...
{/if}
{#if matches('Disable PeX', 'Peer Exchange', 'Client')}
...
{/if}
{#if matches('Disable µTP', 'UDP-based peer connection protocol', 'Client')}
...
{/if}
{#if matches('Disable Auto-Load', 'loading the previously downloaded torrent', 'Client')}
...
{/if}
{#if configTrackers()}
{#if matches('Custom Trackers', 'tracker servers used for peer discovery', 'Client')}
