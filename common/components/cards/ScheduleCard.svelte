...
<div class='schedule-card-ct'
  ...
  <div class='schedule-card pointer load-in' style='--media-color:{mediaColor}; --accent-color:{accentColor}'>
    ...
    <div class='content-col'>
      ...
      {#if sequelInfo}
        <div class='subtitle'>Sequel to {sequelInfo}</div>
      {:else if sourceInfo}
...
<style>
  ...
  .schedule-card {
    ...
    background:hsl(var(--dark-color-light-hsl)); border:1px solid var(--border-color-sp);
  ...
  .cover-link :global(.cover-img) { width:100%; height:100%; object-fit:cover; display:block; transition:transform 0.3s ease; }
  ...
  .cover-title { font-size:1.65rem; font-weight:800; color:#fff; display:-webkit-box; line-height:1.3; letter-spacing:-0.01em; }
  .cover-studio { font-size:0.95rem; font-weight:500; color:var(--accent-color); letter-spacing:0.06em; text-transform:uppercase; opacity:0.85; }
  ...
  .episode-label { font-size:0.9rem; font-weight:500; color:rgba(190,190,210,0.35); letter-spacing:0.08em; text-transform:uppercase; }
  .countdown { font-size:2.4rem; font-weight:900; color:var(--accent-color); line-height:1.4; letter-spacing:-0.03em; }
  ...
  :global(.view-text) .content-col { flex-direction:row; align-items:center; padding:8px 12px; gap:10px; }
  ...
  :global(.view-text) .top-row { flex-direction:row; align-items:center; gap:6px; flex-shrink:0; }
  ...
  :global(.view-text) .stats-col, :global(.view-text) .subtitle,
  :global(.view-text) .description-wrap, :global(.view-text) .genres { display:none; }

  /* hover art (text mode only) */
  .text-hover-art { display:none; }
  :global(.view-text) .text-hover-art {
    display:block; position:fixed; width:200px; height:285px; border-radius:0.7rem;
    overflow:hidden; box-shadow:0 8px 32px rgba(0,0,0,0.75);
    opacity:0; pointer-events:none; z-index:9999;
    transition:opacity 0.15s ease, transform 0.15s ease; transform:scale(0.92);
  }
  ...
  @media (max-width:700px) {
    ...
    .img-col { flex:0 0 80px; width:80px; height:110px; border-radius:6px 0 0 6px; }
    ...
    .episode-label, .countdown { font-size:14px; color:rgba(190,190,210,0.55); letter-spacing:0; text-transform:none; line-height:1.3; }
    .countdown { font-weight:400; }
    .cover-title, .mobile-title { display:block; font-size:15px; font-weight:700; color:#fff; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-bottom:4px; line-height:1.2; }
  }
</style>