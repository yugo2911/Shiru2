...
import { past, since, matchPhrase, monthDay, capitalize } from '@/modules/util.js'
...
export async function dubbedEpisode(i, media) {
...
/**
 * Builds the full episode list for a media entry, resolving titles, air dates,
 * dub/sub scheduling, filler flags and zero episodes. Shared between the
 * Details modal episode list and the player page episode panel.
 * @param {object} media AniList media object.
 * @param {number} episodeCount Maximum episode count.
 */
export async function getEpisodes(media, episodeCount) {
  ...
  let episodeList = Array.from({ length: (newEpisodeCount > episodeCount ? newEpisodeCount : episodeCount) }, (_, i) => ({
    episode: i + 1, image: null, summary: null, rating: null, title: null, length: null, airdate: null, airingAt: null, filler: episodesList.getSingleEpisode(media.idMal, (i + 1)), dubAiring: dubbedEpisode(i, media)
  }))
  ...
  if (!(media.episodes && media.episodes === newEpisodeCount && media.status === 'FINISHED')) {
    ...
    if (settled?.length >= newEpisodeCount) {
      alEpisodes = settled.map((episode, i) => ({
        ...episode, airingAt: episode.airingAt, episode: episode.episode, filler: episodesList.getSingleEpisode(media.idMal, (i + 1)), dubAiring: dubbedEpisode(i, media)
      }))
      ...
      alEpisodes = alEpisodes.map((episode, i) => {
        ...
        if (settledData) return { ...episode, airingAt: settledData.airingAt ?? episode.airingAt, episode: settledData.episode ?? episode.episode, filler: episodesList.getSingleEpisode(media.idMal, (i + 1)), dubAiring: dubbedEpisode(i, media)}
        return episode
      })
    }
  }
  ...
  if ((alEpisodes.length < episodeCount) || (!alEpisodes.length && !episodeCount)) {
    const eps = await episodesList.getEpisodeData(media.idMal)
    if (eps?.length > 0) {
      const lastId = eps[eps.length - 1].episode_id
      alEpisodes = Array.from({ length: (lastId) }, (_, i) => ({
        episode: i + 1, image: null, summary: null, rating: null, title: (lastId <= 100 ? eps.find(e => e.episode_id === (i + 1)) : episodesList.getSingleEpisode(media.idMal, (i + 1)))?.title, length: null, airdate: null, airingAt: (lastId <= 100 ? eps.find(e => e.episode_id === (i + 1)) : episodesList.getSingleEpisode(media.idMal, (i + 1)))?.aired, filler: episodesList.getSingleEpisode(media.idMal, (i + 1)), dubAiring: dubbedEpisode(i, media)
      }))
  ...
  if (zeroEpisode) alEpisodes.unshift({ episode: 0, title: zeroEpisode[0].title, airingAt: media.airingSchedule?.nodes?.find(node => node.episode === 1)?.airingAt || zeroEpisode[0].airingAt, filler: episodesList.getSingleEpisode(media.idMal, 0), dubAiring: dubbedEpisode(0, media)})
  for (const { episode, title: oldTitle, airingAt, filler, dubAiring } of alEpisodes?.length ? alEpisodes : [{ episode: 1, title: null, airingAt: null, filler: null, dubAiring: null }]) {
    ...
    const title = episode === 0 ? oldTitle : newTitle?.en || oldTitle?.en || (await episodesList.getSingleEpisode(media.idMal, episode))?.title || episodeRx.exec(streamingTitle?.title)?.[2]
    lastDuration = length || media.duration || lastDuration
